...
NodeElm.prototype.addSubElement = function (elm) {
    elm = new SubElm(elm, this);
};

NodeElm.prototype.addListener = function (elm) {
    elm.type = Enny.TYPES.listener
    elm = new SubElm(elm, this);
    if (this.subelements[elm.id]) {
        return this.subelements[elm.id];
    }
    this.subelements[elm.id] = elm;
};
